...
canonical JSON reports). On top of them the thin CLI adapter
...
implementation and rot-guarded in CI). Properties (canonicalization
idempotence, Merkle proof soundness) are pinned by mutation-verified
property suites, and throughput is tracked by `moon bench` suites (see
Performance above).
...
All commands pass locally (131/131 unit tests, 22/22 black-box cases) as of
2026-06-11 Asia/Shanghai; CI additionally builds the native CLI and runs the
same black-box suite against it. Next up: three-backend delivery and the
browser demo (master plan step 9).
