...
- **native timing 探针**：dudect-style Ed25519 verify/sign 采样器，面向本机 native release 构建输出 Welch t；只作为经验性侧信道证据，不当作形式化证明
...
截至 2026-07-07 Asia/Shanghai，native/wasm-gc/js 本地测试基线全绿；native 已在 Windows 的 MSVC 19.44 + Windows SDK 10.0.26100.0 环境验证。代码库共 13925 有效 MoonBit 行（实现 5876 + 测试 8049），实现规模仍处于竞赛 4-10k 区间内。
