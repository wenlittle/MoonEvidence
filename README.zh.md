...
- **337 个单元测试**声明（333 个可执行测试 + 4 个基准 wrapper）；wasm-gc/js 本地全绿，native 由 CI 覆盖；NIST SHA-256/512 向量、RFC 8785 Appendix B 向量全过
...
截至 2026-07-06 Asia/Shanghai，wasm-gc/js 本地测试基线全绿；native 需要 C 编译器，由 CI 覆盖。代码库共 12520 有效 MoonBit 行（实现 5448 + 测试 7072），实现规模仍处于竞赛 4-10k 区间内。
