...
- **可锚定**：Go Chaincode + TypeScript Gateway 已在双组织 Fabric 网络实跑，链上摘要可直接回灌 MoonBit 验证。
...
`demo/web/` 使用同一个 release JS 产物；可选 Fabric 路径消费同一份规范摘要和验证报告，不维护第二套证据语义。
...
graph TD
  ...
  subgraph ledger["可选 Fabric 锚定"]
    GATEWAY["TypeScript Gateway<br/>anchor-pack / verify-anchor"]
    CHAINCODE["Go Chaincode<br/>不可变 manifest 摘要"]
    FABRIC["Fabric 账本<br/>交易 · 区块 · 状态"]
  end
...
| 包 | 入口 | 作用 |
| --- | --- | --- |
| `cmd/main` | CLI：`pack` / `inspect` / `verify` / `explain` / `create` | 命令行工具，版本化 JSON，冻结退出码 0/1/2 |
| `api` | `verify_evidence(request_json)` | 浏览器边界：JSON 字符串进出，零 MoonBit 类型跨界 |
| `integrations/fabric` | `anchor-pack` / `verify-anchor` | TypeScript Gateway + Go Chaincode，只把已验证 manifest 摘要送入 Fabric |

## Hyperledger Fabric 实链锚定

这条可选链路已经跨过真实 Gateway、背书、排序、提交验证、世界状态与双组织查询边界：

```text
MoonBit pack/verify -> TypeScript Gateway -> Go Chaincode -> Fabric 账本
                    <- 链上查询 <- 外部摘要回灌验证
```

2026-07-11 的双组织实验把内置 golden pack 以 `VALID` 状态提交到第 6 区块；Org1 与 Org2 查询到完全相同的不可变记录，Org2 重复提交没有覆盖首笔交易 ID。链上摘要回灌后，原包通过；只改文件得到且仅得到 `E2003`；改文件并重建 manifest 后，对照旧链上摘要得到且仅得到 `E2004`。

部署、配置、CLI 和 Node.js 调用方式见 [Fabric 集成指南（源码仓库）](https://github.com/wenlittle/MoonEvidence/blob/main/integrations/fabric/README.md)，脱敏交易收据见 [真实 E2E 记录](https://github.com/wenlittle/MoonEvidence/tree/main/docs/records/fabric-e2e/2026-07-11)。
...
### 证据包创建与扩展
- **证据包创建**：`pack` / `seal` 一条命令生成完整 `manifest.json + files/`；`create` 保留给已有目录布局
- **机器接口**：`pack` / `create` / `inspect` 输出版本化 JSON，`--expected-manifest-digest` 接收链上或归档系统摘要
...
- **Fabric 摘要锚定**：不可变 Chaincode、提交状态收据、重复交易归一化与双组织回灌验证

## 测试与质量

- **351 个单元测试**声明（347 个可执行测试 + 4 个基准 wrapper）；native/wasm-gc/js 本地全绿；NIST SHA-256/512 向量、RFC 8785 Appendix B 向量全过
- **62 用例 CLI 黑盒套件**：原 54 个验证/创建合同 + 8 个机器接口与外部锚点用例，PowerShell 与 bash 1:1 对等
- **Fabric 集成测试**：Go Chaincode 语句覆盖率 82.1%，TypeScript Gateway 19/19；双组织上链、查询、重复提交与篡改回灌真实 E2E 已留存
...
截至 2026-07-11 Asia/Shanghai，native/wasm-gc/js 本地测试基线全绿；native 已在 Windows 的 MSVC 19.44 + Windows SDK 10.0.26100.0 环境验证。代码库共 14571 有效 MoonBit 行（实现 6453 + 测试 8118），实现规模仍处于竞赛 4-10k 区间内。

## 项目文档

### 入门
- [用户指南（三个真实场景）](docs/GUIDE.md)
- [Hyperledger Fabric 集成（源码仓库）](https://github.com/wenlittle/MoonEvidence/tree/main/integrations/fabric)
...
- [CLI 机器接口契约](docs/spec/CLI_MACHINE_CONTRACT.md)
- [Fabric 锚定契约](docs/spec/FABRIC_ANCHOR_SPEC.md)
