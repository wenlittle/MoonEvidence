...
- **344 个单元测试**声明（340 个可执行测试 + 4 个基准 wrapper）；wasm-gc/js 本地全绿，native 由 CI 覆盖；NIST SHA-256/512 向量、RFC 8785 Appendix B 向量全过
- **53 用例 CLI 黑盒套件**：12 命令形状 + 10 包篡改矩阵 + 19 错误码夹具 + 9 create + 3 incremental（独立 Node 参考实现生成，CI 防腐化校验）
...
截至 2026-07-06 Asia/Shanghai，wasm-gc/js 本地测试基线全绿；native 需要 C 编译器，由 CI 覆盖。代码库共 12607 有效 MoonBit 行（实现 5448 + 测试 7159），实现规模仍处于竞赛 4-10k 区间内。
