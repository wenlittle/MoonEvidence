...
可选 Fabric 适配器遵循最小披露和单一语义来源原则：

- MoonBit CLI 是 manifest 创建、RFC 8785 规范化、摘要、Merkle 与本地验证的唯一语义实现；Go/TypeScript 不重算这些值。
- Chaincode 的唯一客户端业务参数是规范 `sha256:` / `sha512:` manifest 摘要。原文件、文件名/路径、逐文件摘要、Merkle 叶子、完整 manifest 和本地审计日志不得进入交易参数或世界状态。
- 首笔 anchor 不可更新/删除；顺序重复返回原记录，并发 MVCC loser 只有在查询到相同锚点后才被 Gateway 归一化。
- 提交证据采用 Fabric transaction ID、block number 和 validation status。提议时间由客户端事务头提供，不作为独立可信时钟或法律时间戳。
- 真实证书、私钥与 connection profile 只能放在 Git 忽略的本地目录或密钥管理系统。错误 envelope 和提交记录不得回显凭据内容。

本地双组织 test-network 证明协议可运行；生产部署需要组织级 CA enrollment、密钥托管、通道/背书治理、节点可用性和账本备份策略。
...
本安全策略适用于 MoonEvidence 仓库内的源代码（`src/`）、工具脚本（`tools/`）与可选集成（`integrations/`）。`tests/fixtures/` 与 `examples/` 中的数据为测试/演示用途，不构成安全边界。
