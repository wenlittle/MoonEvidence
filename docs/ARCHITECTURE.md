...
依赖从入口向核心和基础能力单向流动。纯核心接收文本、字节和结构化参数，不读取文件，不启动进程，也不连接网络。Go 和 TypeScript 适配器消费 MoonBit 产生的 manifest 回执和验证报告，不复制证据语义。
...
| `integrations/fabric/chaincode-go` | 验证规范摘要，按 v1 规则保存首笔记录 | 不处理文件或 manifest |
...
| 可选版本链 | 调用方 | `versions/version_chain.json` | 表达连续发布关系 |
...
5. 成功结果包含核心证据包和版本化 JSON 回执；可选版本链由调用方按发布关系写入。
...
包含性证明使用独立的 `generate_proof`、`verify_proof` 浏览器 API 和 `merkle` 包接口。它们不进入每次证据包完整验证，也不要求证据包携带 `proofs/` 目录。
...
Chaincode v1 没有更新或删除交易，顺序重复返回首笔记录；并发首写只有在 Fabric 返回 MVCC code 11 且查询到相同记录时才归一化。Fabric 通道治理控制账本生命周期，提交回执在链下保存交易 ID、区块号和验证状态。
...
| 新浏览器能力 | 增加 JSON 字符串 API、稳定响应字段和接口测试 | 后台线程只传 JSON 和可复制数据 |
...
| Fabric 链码 | Go | 首笔摘要记录，v1 无更新或删除交易 |
...
| 进程合同区分回执和报告 | TypeScript 网关校验 `pack`/`inspect` 版本化回执，并校验 `verify` 的稳定诊断报告 |
