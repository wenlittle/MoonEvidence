...
> 前置（演示前完成，不计时）：根目录执行 `moon build --target js`；`showcase/` 目录执行 `npm ci`、`npm run build`、`npm run preview`；浏览器开好 `http://localhost:4173/`；Fabric 环节使用已保存的双组织交易记录，现场网络可用时再执行查询命令。
