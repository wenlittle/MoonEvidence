...
> 目标：让评委在 5 分钟内看到“承诺 → 篡改 → 抓住 → 解释”的完整闭环，外加三后端与 Trust Observatory 亮点。
...
## 第 3 分钟 · Trust Observatory 交互式观测台

切到已开好的 Trust Observatory（`http://localhost:4173/`）：

1. **自动模式**：时间轴依次展开文件、规范化、摘要、Merkle 根、Ed25519 签名与外部锚点载荷；拖到“拒绝”，原始与篡改两条证据路径同屏对照。
2. **挑战模式**：点“挑战”，选择 `files/a.txt`，页面立即展示修改字节、分叉摘要、不同 Merkle 根、签名拒绝和 `E2003` 诊断。
3. **检查模式**：暂停播放并点击证据文件节点，查看选择高亮和真实摘要。
4. **证据工作台**：点击顶部“证据工作台”，同站进入验证、创建、证明、审计、签名和篡改实验六项可运行工具，再点“观测首页”返回。
5. 强调：场景中的摘要、根、签名和错误码全部由 Web Worker 调用编译后的 MoonBit API 现场计算，输入就是仓库的 `examples/valid-pack`；文件没有上传。
...
- [ ] Trust Observatory 已在 `http://localhost:4173/` 打开过一次（MoonBit API 与场景资源已缓存）
- [ ] 终端字体够大；`$cli` 变量已设
