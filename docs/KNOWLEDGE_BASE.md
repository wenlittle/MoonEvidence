...
| 代码规模 | 12668 有效 MoonBit 行（实现 5448 + 测试 7220） |
| 测试 | 348 个测试声明（344 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
...
| 测试函数总数 | 348 个声明（344 个可执行测试 + 4 个基准 wrapper） |
...
| `timing-ed25519-verify.mjs` | Ed25519 verify 动态时序采样。比较两类同形 valid verify 请求，输出均值、方差和 Welch t 统计 | 默认 10000 samples；发布/审计时手动运行；信息性探针，不是 dudect 证明 |
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"348个测试声明"仍是数量非覆盖 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 348个声明（344可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto/create/store/audit/api 分支图 + stale-check gate + API malformed fuzz gate + API semantic property gate + CLI_VERSION gate | 继续补 release/manual 长轮次 fuzz 与 dudect/后端产物级侧信道验证 |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
...
| `tools/timing-ed25519-verify.mjs` | Ed25519 verify 动态时序采样探针 |
