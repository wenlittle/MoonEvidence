...
| 代码规模 | 12520 有效 MoonBit 行（实现 5448 + 测试 7072） |
| 测试 | 337 个测试声明（333 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
...
| 测试函数总数 | 337 个声明（333 个可执行测试 + 4 个基准 wrapper） |
...
| 变异测试 | 16 个变异点（全部被捕获） |
| 分支清单审计 | `docs/BRANCH_COVERAGE.md` 首批覆盖 verify/incremental/merkle/digest/crypto 93 个关键分支，0 个 open gap |
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"337个测试声明"仍是数量非覆盖 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 337个声明（333可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto 首批分支图 | 扩展分支审计到 create/store/audit |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
...
| `docs/BRANCH_COVERAGE.md` | 手工分支覆盖审计图：当前覆盖 verify/incremental/merkle/digest/crypto 关键接受/拒绝/跳过/警告分支 |
