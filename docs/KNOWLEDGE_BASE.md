...
| 代码规模 | 13925 有效 MoonBit 行（实现 5876 + 测试 8049） |
| 测试 | 348 个测试声明（344 个可执行测试 + 4 个基准 wrapper）+ 54 例黑盒 CLI 套件 |
...
| 黑盒测试 | 54 例 CLI 套件（`cli-test.ps1` / `cli-test.sh`） |
...
| H4 | 常量时间审计、CT-001 修复与 native timing | `field25519.mbt:199,214`; `point25519.mbt:129,144`; `ed25519.mbt:81-153`; `src/timing`; `docs/CONST_TIME_AUDIT.md` | 静态审计确认 `scalar_mul`/`cmov` 源码层面无明显 secret-dependent 分支；`reduce_scalar_512` 的比较与 borrow 分支已改为 arithmetic mask/selection。native 50000 样本长跑未观察到明显 timing difference；仍需正式 dudect/后端产物审计后才能作生产级侧信道声明。 |
...
| bash cli-test 对齐 | 2026-07-06 已补齐：`cli-test.sh` 与 `cli-test.ps1` 均为 54 例，覆盖 Part 4 create + Part 5 incremental + create depth-cap abort |
...
| 13 | `pwsh ./tools/cli-test.ps1 -Target native` | CLI 黑盒（54例，native） | — |
| 14 | `moon build --target wasm-gc` | 构建 wasm-gc | — |
| 15 | `moon build --target js` | 构建 js | — |
| 16 | `pwsh ./tools/cli-test.ps1 -Target js` | CLI 黑盒（54例，js） | — |
...
| `timing-ed25519-verify.mjs` | Ed25519 verify JS 动态时序采样。比较两类同形 valid verify 请求，输出均值、方差和 Welch t 统计 | 默认 10000 samples；发布/审计时手动运行；信息性探针，不是 dudect 证明 |
| `timing-ed25519-native.ps1` | Ed25519 native dudect-style timing runner。构建 `src/timing` native release，覆盖 verify/sign-message/sign-secret，输出均值、方差、Welch t 与环境信息 | 50k 长跑已记录为本机经验性侧信道证据；不是形式化证明或正式 dudect 审计 |
...
| # | 限制 | 风险 |
|---|---|---|
| 1 | 恒定时间实现为源码审计 + 本机 native timing 经验性证据 | `docs/CONST_TIME_AUDIT.md` 已记录 CT-001 源码级修复与 50000 样本 native timing；尚未经正式 dudect/专业审计证明 |
...
> 本项目密码学实现（尤其是 Ed25519）**尚未经过外部专业安全审计**。通过 RFC 8032 KAT、Wycheproof、交叉对拍、源码级常量时间审计与本机 native dudect-style timing 长跑，并已补齐反可塑性/恒定时间标量乘法/branch-free 源码级标量归约/binary quotient decomposition/低阶点拒绝等防护；但尚未经正式 dudect 审计和后端产物审计证明，**不应在生产环境或高价值资产保护场景中作为唯一信任根**。
...
| js | ✅ | ✅（与 wasm-gc 同步） | ✅ | ✅（54例） | 浏览器适配器烟测 + 变异测试也用 js |
| native | ✅ | ✅ | ✅ | ✅（54例） | 本地 Windows/MSVC 已验证；CI ubuntu/gcc 继续覆盖 |
...
| C: 密码学测试未对标行业标准 | H2,H3,H4 | Wycheproof Ed25519 已补 150 向量；静态常量时间审计已补，CT-001 已源码级修复；native dudect-style timing 已补；仍缺正式 dudect/后端产物审计 |
| D: 安全函数测试优先级低 | M2,M3 | 功能先测，防篡改安全函数后测 |
| E: 测试资产双轨漂移 | 6项治理缺口 | ps1/sh 已对齐到 54 例；仍需避免未来人工移植再次漂移 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 348个声明（344可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto/create/store/audit/api 分支图 + stale-check gate + API malformed fuzz gate + API semantic property gate + CLI_VERSION gate | 继续按高风险变更补 oracle/mutation；侧信道保留正式 dudect/后端产物级审计边界 |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
| L2 | 属性测试 | 3个 | +8（Ed25519/Fe/canonjson） |
| L3 | 差分测试 | 固定夹具 | +随机差分harness |
| L4 | CLI黑盒 | ps1=54,sh=54 | 后续改 CLI 时双脚本同步 |
| L5 | 变异测试 | 16点 | 继续按高风险改动补点 |
| L6 | 模糊测试 | 无 | 10000轮不崩溃 |
| L7 | 性能基准 | 仅js | +native/wasm-gc |
| L8 | 安全测试 | 源码审计 + JS timing + native verify/sign timing | 正式 dudect/后端产物审计前不作生产级侧信道声明 |
