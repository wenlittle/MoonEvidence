...
| 代码规模 | 12607 有效 MoonBit 行（实现 5448 + 测试 7159） |
| 测试 | 344 个测试声明（340 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
| CI | 18+ 步主流水线，4 道防线（指标/漂移门禁 + 交叉对拍 + API fuzz + 变异测试） |
...
JS ESM 导出（`moon.pkg` 中 `link.js.exports`）：`verify_evidence`, `compute_merkle_tree`, `create_evidence_pack`, `generate_proof`, `verify_proof`, `audit_append`, `audit_verify`, `audit_sign`, `ed25519_keypair`, `ed25519_sign`, `ed25519_verify`
...
| 常量 | 值 | 说明 |
|---|---|---|
| `CLI_VERSION` | `"0.4.0"` | 与 `moon.mod` 同步（**无 CI 自动校验**） |
| `max_pack_depth` | `32` | 目录递归深度上限（防 symlink-to-dir 循环） |
...
```
moon-evidence verify [--json] [--incremental <cache-dir>] <path> [<path> ...]
...
| `--version` | `-v`, `version` | 打印 `moon-evidence 0.4.0` |
| `--help` | `-h`, `help` | 打印用法 |
...
| API | 请求 JSON | 响应 JSON |
|---|---|---|
| `verify_evidence` | `{manifest, files?, version_chain?}` | `{ok, explain, report}` 或 `{ok:false, error}` |
...
| 测试函数总数 | 344 个声明（340 个可执行测试 + 4 个基准 wrapper） |
| 黑盒测试 | 53 例 CLI 套件（`cli-test.ps1` / `cli-test.sh`） |
| 属性测试 | 3 个文件（canonjson_prop, merkle_prop, manifest fuzz） |
| 基准测试 | 2 个文件（digest_bench, verify_bench） |
| 变异测试 | 16 个变异点（全部被捕获） |
| 分支清单审计 | `docs/BRANCH_COVERAGE.md` 覆盖 verify/incremental/merkle/digest/crypto/create/store/audit 145 个关键分支，0 个 open gap |
...
| H4 | 常量时间审计与 CT-001 修复 | `field25519.mbt:199,214`; `point25519.mbt:129,144`; `ed25519.mbt:81-153`; `docs/CONST_TIME_AUDIT.md` | 静态审计确认 `scalar_mul`/`cmov` 源码层面无明显 secret-dependent 分支；`reduce_scalar_512` 的比较与 borrow 分支已改为 arithmetic mask/selection。仍需 dudect/后端产物审计后才能作生产级侧信道声明。 |
...
| L4 | Fe::from_small 无直接测试 | `field25519.mbt:27` | 内部辅助函数无直接单元测试。 |
| L5 | Fe::to_bytes 条件减 p | `field25519.mbt:161-186` | 常量时间条件减 p 的 borrow 分支未被直接测试。 |
| L6 | diag 单复数分支 | `diag.mbt:139, 141` | "1 error" vs "errors" 和 "1 warning" vs "warnings" 分支未测试。 |
| L7 | digest parse_digest 失败分支 | `digest.mbt:119-127` | 三个 guard 失败分支（无冒号、未知算法、非 hex）未被直接测试。 |

### 9.4 已知治理缺口

| 缺口 | 说明 |
|---|---|
| CLI_VERSION 一致性无 CI 门禁 | `main.mbt` 中硬编码的 `CLI_VERSION = "0.4.0"` 与 `moon.mod` version 的一致性靠人工同步 |
| E3002 错误码未覆盖 | CLI 不提供 proofs/ 消费者，无 manifest 夹具能触发 E3002 |
| bash cli-test 对齐 | 2026-07-06 已补齐：`cli-test.sh` 与 `cli-test.ps1` 均为 53 例，覆盖 Part 4 create + Part 5 incremental |
| 基准测试仅 js 后端 | native 后端预期更快但未实测 |
| 无 fuzzing 配置 | 变异测试是最接近 fuzzing 的质量门禁，但非随机 fuzz |
| 符号链接安全缓解为运行时上限 | `@fs` 包不暴露 symlink 检测 API，只能用深度+文件数双上限 |
...
- **运行环境**：`ubuntu-latest`
- **权限**：`contents: read`
- **两个 Job**：`check-test-build`（阻塞性）+ `bench`（非阻塞）
...
| 13 | `pwsh ./tools/cli-test.ps1 -Target native` | CLI 黑盒（53例，native） | — |
...
| 19 | `node tools/differential-crypto.mjs --rounds 64` | Ed25519 JS API 与 Node.js crypto 随机差分对拍 | 第2道 |
| 20 | `node tools/differential-digest.mjs --rounds 64` | SHA-256/SHA-512/HMAC-SHA256 JS API 与 Node.js crypto 随机差分对拍 | 第3道 |
| 21 | `node tools/mutation-check.mjs` | **变异测试**：逐字节改写源码，确认测试变红 | 第4道 |
...
| 文件 | 用途 | 关键参数/机制 |
|---|---|---|
| `check-metrics.mjs` | 指标漂移门禁。19 条断言校验文档数字与仓库实测一致 | `--fix` 打印建议修复（不写入）。提交数用 `minimum` 语义 |
| `check-branch-coverage-stale.mjs` | 分支覆盖图防漂移门禁。已审计源码变更必须同步触碰 `docs/BRANCH_COVERAGE.md` | 支持 `--self-test`；CI 中按 PR/base diff 自动判断 |
...
| 文件 | 用例数 | 说明 |
|---|---|---|
| `cli-test.ps1` | 53 | PowerShell 黑盒套件，5部分（命令形状12 + 篡改矩阵10 + manifest错误码19 + create9 + incremental3）。`-Target js\|native` |
| `cli-test.sh` | 53 | bash 1:1 对等移植版（Part1-5）。**强制依赖 jq**；CI 同时跑 native/js |
...
| 5 | Identity point 拒绝 | 显式拒绝 identity 公钥，阻断 R = S·B 伪造路径 |
...
| # | 限制 | 风险 |
|---|---|---|
| 1 | 恒定时间实现为代码审查级别 | `docs/CONST_TIME_AUDIT.md` 已记录 CT-001 源码级修复；尚未经正式侧信道分析工具（如 dudect）量化验证 |
| 2 | MoonBit 编译器不承诺消除 secret-dependent 内存访问 | native 后端的 C 编译器可能重新引入分支 |
| 3 | 低阶点检查不完整 | 仅覆盖 identity point，未实现完整 cofactor 8 乘法检查（7个非identity低阶点可能通过 `point_decode`） |
| 4 | 符号链接安全缓解为运行时上限 | `@fs` 包不暴露 symlink 检测 API，只能用 `max_pack_depth=32` + `max_pack_files=10000` |

### 13.4 重要声明

> 本项目密码学实现（尤其是 Ed25519）**尚未经过外部专业安全审计**。通过 RFC 8032 KAT 与交叉对拍，并已补齐反可塑性/恒定时间标量乘法/branch-free 源码级标量归约/binary quotient decomposition/低阶点拒绝等防护；但尚未经 dudect 等侧信道工具和后端产物审计验证，**不应在生产环境或高价值资产保护场景中作为唯一信任根**。
...
| js | ✅ | ✅（与 wasm-gc 同步） | ✅ | ✅（53例） | 浏览器适配器烟测 + 变异测试也用 js |
| native | ✅ | ✅ | ✅ | ✅（53例） | CI 专属（本地 Windows 无 gcc） |

### 14.3 关键差异

1. **native 后端需要 C 编译器**：本地开发机（Windows）无 gcc，native 构建/测试/黑盒全部委托给 CI
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"344个测试声明"仍是数量非覆盖 |
| C: 密码学测试未对标行业标准 | H2,H3,H4 | Wycheproof Ed25519 已补 150 向量；静态常量时间审计已补，CT-001 已源码级修复；仍缺 dudect/动态侧信道验证 |
| D: 安全函数测试优先级低 | M2,M3 | 功能先测，防篡改安全函数后测 |
| E: 测试资产双轨漂移 | 6项治理缺口 | ps1/sh 已对齐到 53 例；仍需避免未来人工移植再次漂移 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 344个声明（340可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto/create/store/audit 分支图 + stale-check gate | 继续补 API fuzz/property |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
| L2 | 属性测试 | 3个 | +8（Ed25519/Fe/canonjson） |
| L3 | 差分测试 | 固定夹具 | +随机差分harness |
| L4 | CLI黑盒 | ps1=53,sh=53 | 后续改 CLI 时双脚本同步 |
...
| 文件路径 | 用途 |
|---|---|
| `docs/ARCHITECTURE.md` | 分层架构 + 冻结 API 签名（v1 + v2） |
...
| `docs/BRANCH_COVERAGE.md` | 手工分支覆盖审计图：当前覆盖 verify/incremental/merkle/digest/crypto/create/store/audit 关键接受/拒绝/跳过/警告分支 |
...
| `tools/cli-test.ps1` | PowerShell 黑盒（53例） |
| `tools/cli-test.sh` | bash 黑盒（53例） |
...
| 2026-07-06 | 修复 CT-001：`reduce_scalar_512` 比较/borrow 改为 arithmetic mask/selection，常量时间声明更新为源码审计级，仍保留 dudect/后端验证限制 | Codex |
