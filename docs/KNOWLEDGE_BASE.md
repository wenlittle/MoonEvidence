...
| 怎么系统化深度测试 | [§15 系统化深度测试计划](#15-系统化深度测试计划)（详见 [TEST_PLAN.md](./TEST_PLAN.md)） |
...
| 代码规模 | 11977 有效 MoonBit 行（实现 5395 + 测试 6582） |
| 测试 | 308 个测试声明（304 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
...
| 测试函数总数 | 308 个声明（304 个可执行测试 + 4 个基准 wrapper） |
...
| `canonjson` | 3 | 31 | 良好 |
| `crypto` | 4 | 39 | Wycheproof oracle 已补，仍需精确分支覆盖 |
| `merkle` | 3 | 30 | 良好（大规模树未测） |
| `audit` | 1 | 22 | 良好 |
| `create` | 1 | 10 | 有 abort 分支盲点 |
...
| H4 | 常量时间无验证 | `field25519.mbt:199,214`; `point25519.mbt:129,144`; `ed25519.mbt:106-144` | 所有标注为常量时间的函数（`fe_cmov`, `point_cmov`, `Fe::eq`, `scalar_mul`, `reduce_scalar_512`）均无计时测试。无法验证是否真正常量时间。 |
...
| M4 | 大规模 Merkle 树 | `merkle.mbt:80-105` | 测试仅覆盖到 5 叶，缺少 1024+ 叶的边界测试。 |
...
| bash cli-test 缺少 12 个用例 | `cli-test.sh` 41 例 vs `cli-test.ps1` 53 例（缺 Part 4 create + Part 5 incremental） |
...
| 7 | `moon check` | 类型检查（0 warning） | — |
| 8 | `moon fmt --check` | 格式化门禁 | — |
| 9 | `moon test --target wasm-gc,js` | wasm-gc + js 单元测试 | — |
| 10 | `moon build --target native` | 构建 native | — |
| 11 | `moon test --target native` | native 单元测试 | — |
| 12 | `pwsh ./tools/cli-test.ps1 -Target native` | CLI 黑盒（53例，native） | — |
| 13 | `moon build --target wasm-gc` | 构建 wasm-gc | — |
| 14 | `moon build --target js` | 构建 js | — |
| 15 | `pwsh ./tools/cli-test.ps1 -Target js` | CLI 黑盒（53例，js） | — |
| 16 | `node tools/smoke-api.mjs` | 浏览器适配器烟测（11个API） | — |
| 17 | `node tools/mutation-check.mjs` | **变异测试**：逐字节改写源码，确认测试变红 | 第3道 |
...
| `cli-test.sh` | 41 | bash 1:1 对等移植版（Part1-3）。**强制依赖 jq** |
...
| # | 限制 | 风险 |
|---|---|---|
| 1 | 恒定时间实现为代码审查级别 | **尚未经正式侧信道分析工具（如 dudect）量化验证** |
...
> 本项目密码学实现（尤其是 Ed25519）**尚未经过外部专业安全审计**。通过 RFC 8032 KAT 与交叉对拍，并已补齐反可塑性/恒定时间/binary quotient decomposition/低阶点拒绝等防护，但**不应在生产环境或高价值资产保护场景中作为唯一信任根**。
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"308个测试声明"仍是数量非覆盖 |
| C: 密码学测试未对标行业标准 | H2,H3,H4 | Wycheproof Ed25519 已补 150 向量；仍缺 dudect/动态侧信道验证 |
| D: 安全函数测试优先级低 | M2,M3 | 功能先测，防篡改安全函数后测 |
| E: 测试资产双轨漂移 | 6项治理缺口 | ps1(53例) vs sh(41例) 人工移植无共享源 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 308个声明（304可执行+4基准wrapper） | +剩余分支/错误路径测试 |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
| L2 | 属性测试 | 3个 | +8（Ed25519/Fe/canonjson） |
| L3 | 差分测试 | 固定夹具 | +随机差分harness |
| L4 | CLI黑盒 | ps1=53,sh=41 | sh补齐到53 |
| L5 | 变异测试 | 8点 | 13-16点 |
| L6 | 模糊测试 | 无 | 10000轮不崩溃 |
...
| 阶段 | 优先级 | 内容 | 工作量 | 前置条件 |
|---|---|---|---|---|
| **阶段1** | P0阻断 | 已完成 Wycheproof Ed25519 oracle、store 安全函数 oracle、增量缓存信任边界；剩余 Ed25519 精确分支、常量时间审计、create abort | 继续收口 | 无 |
