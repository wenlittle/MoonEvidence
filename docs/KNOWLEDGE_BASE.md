...
| 代码规模 | 12429 有效 MoonBit 行（实现 5448 + 测试 6981） |
| 测试 | 329 个测试声明（325 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
...
| 测试函数总数 | 329 个声明（325 个可执行测试 + 4 个基准 wrapper） |
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"329个测试声明"仍是数量非覆盖 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 329个声明（325可执行+4基准wrapper） | +剩余分支/错误路径测试 |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
