...
| 代码规模 | 12464 有效 MoonBit 行（实现 5448 + 测试 7016） |
| 测试 | 331 个测试声明（327 个可执行测试 + 4 个基准 wrapper）+ 53 例黑盒 CLI 套件 |
...
| 测试函数总数 | 331 个声明（327 个可执行测试 + 4 个基准 wrapper） |
...
| 变异测试 | 16 个变异点（全部被捕获） |
| 分支清单审计 | `docs/BRANCH_COVERAGE.md` 首批覆盖 verify/incremental/merkle 45 个关键分支，0 个 open gap |
...
| `api` | 1 | 39 | 最全面 |
| `model` | 2 | 34 | 良好 |
| `digest` | 6 | 41 | 良好（SHA-512 有缺口） |
| `verify` | 4 | 40 | 良好（增量验证独立 oracle 已补，错误路径仍可扩展） |
| `canonjson` | 3 | 31 | 良好 |
| `crypto` | 4 | 47 | Wycheproof oracle 与精确分支覆盖已补；常量时间静态审计已完成，CT-001（`reduce_scalar_512` secret-derived 分支）已源码级修复 |
| `merkle` | 3 | 32 | 良好（边界/大规模/SHA-512 域分离已补） |
| `audit` | 1 | 22 | 良好 |
| `create` | 1 | 15 | 5 个 panic 测试覆盖 create_manifest abort 分支 |
| `store` | 1 | 16 | 安全函数独立 oracle 已补 |
| `diag` | 1 | 6 | 有单复数分支盲点 |
| **合计** | **27** | **331** | — |
...
| B: 无分支覆盖率模型 | H2,H3,L2-L6 | MoonBit 缺 coverage 工具，"331个测试声明"仍是数量非覆盖 |
...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 331个声明（327可执行+4基准wrapper）+ verify/incremental/merkle 首批分支图 | 扩展分支审计到 digest/crypto/create/store/audit |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
...
| `docs/BRANCH_COVERAGE.md` | 手工分支覆盖审计图：当前覆盖 verify/incremental/merkle 关键接受/拒绝/跳过/警告分支 |
