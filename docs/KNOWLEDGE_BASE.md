...
| 层次 | 名称 | 现状 | 目标 |
|---|---|---|---|
| L0 | 单元测试 | 344个声明（340可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto/create/store/audit/api 分支图 + stale-check gate + API malformed fuzz gate | 继续补 CLI_VERSION gate 与 API semantic property |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） |
