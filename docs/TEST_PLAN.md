...
| CLI 黑盒合同 | `powershell -ExecutionPolicy Bypass -File tools/cli-test.ps1 -Target native/js`; `bash ./tools/cli-test.sh native/js` | PASS：native 53/53 + js 53/53（PowerShell 与 bash） | 每次 PR / 发布候选 |
...
剩余边界也固定为文档化风险：Ed25519 侧信道仍缺 dudect/后端产物级证明；`@fs` 无 lstat/symlink API，符号链接语义只能靠 depth/file cap 缓解；随机测试是高强度抽样，不是全输入证明；CI 仍需在远端 ubuntu/gcc 环境确认 native gate。
