...
`tools/mutation-check.mjs` 对安全不变量注入 18 个实现故障。当前测试全部捕获，覆盖 Merkle 域分离和奇数节点、Ed25519 编码和低阶点、HMAC/SHA-2 常量、增量验证的外部摘要、审计签名和对象存储内容哈希。
...
| 故障注入 | 18 个定向变异 | `18/18` 捕获，源码恢复检查通过 |
