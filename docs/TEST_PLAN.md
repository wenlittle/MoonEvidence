...
| 3.5 | 动态时序测量（Ed25519验证10000次采样+统计） | L8 | Done: `tools/timing-ed25519-verify.mjs --samples 10000` 已实跑并记录 Welch t=-0.118306；明确不是 dudect 证明。50000 样本在本机约一小时级，不作为当前门禁 |
...
剩余边界也固定为文档化风险：Ed25519 侧信道仍缺 dudect/后端产物级证明；`@fs` 无 lstat/symlink API，符号链接语义只能靠 depth/file cap 与 create depth-cap abort 缓解；随机测试是高强度抽样，不是全输入证明；CI 仍需在远端 ubuntu/gcc 环境确认 native gate。
