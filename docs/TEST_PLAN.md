...
> **2026-07-07 进度记录**：Phase 1-3 已阶段性收口：Wycheproof Ed25519 150 向量、Ed25519 精确分支 8 用例、store 完整性/严格重建 6 个独立 oracle、incremental golden manifest 5 个独立 oracle（含 Q3 缓存信任边界）、Ed25519 常量时间静态审计、create_manifest 5 个 panic 错误路径测试、CT-001 源码级修复、Ed25519/SHA/HMAC 随机差分 oracle、公共 JS API malformed fuzz 与 semantic property、Merkle 大规模/边界测试、mutation 16/16 捕获、verify/incremental/merkle/digest/crypto/create/store/audit/api 分支审计和 stale-check 门禁、CLI_VERSION 一致性门禁、Phase 3 低风险分支与 native timing harness。当前发布候选基线：`moon test --target native` 为 344/344 passed；`moon test --target js` 为 344/344 passed；`moon test --target wasm-gc` 为 344/344 passed；CLI PowerShell/bash native 与 JS 黑盒均为 54/54 passed；`randomized-hardening` stress 四段已分段实跑通过（10000 malformed、1000 semantic、5000 Ed25519 differential、5000 digest differential）；`check-metrics` 口径为 348 个测试声明（344 可执行测试 + 4 基准 wrapper）。常量时间结论已从纯源码审计推进到本机 native dudect-style timing evidence，但仍不是形式化证明、正式 dudect 审计或后端机器码级证明。
...
| 限制 | 说明 |
|---|---|
| **H4 常量时间计时测试成本高** | 已补本机 native dudect-style harness；生产级声明仍需正式 dudect/后端机器码审计，成本高 |
...
| **C: 密码学测试未对标行业标准** | H2, H3, H4 | RFC 8032 §7.1 KAT + Wycheproof Ed25519 150 向量已覆盖签名 oracle；native dudect-style timing 已补，仍缺正式 dudect/后端产物级审计 |
| **D: 安全函数测试优先级被低估** | M2, M3 | 功能路径先测，防篡改/防绕过的安全函数后测甚至不测 |
| **E: 测试资产双轨漂移** | 6项治理缺口 | cli-test.ps1 与 cli-test.sh 已对齐到 54 例；后续仍需防止人工移植漂移 |
...
| 密码学测试三支柱 | KAT + Wycheproof/攻击向量 + native timing 经验性证据均已建立 | 仍需正式 dudect/后端产物审计，才能从工程证据升级到生产级侧信道声明 |
...
| 层次 | 名称 | 现状 | 目标 | 优先级 |
|---|---|---|---|---|
| L0 | 单元测试（白盒） | 348个声明（344可执行+4基准wrapper）+ verify/incremental/merkle/digest/crypto/create/store/audit/api 分支图 + stale-check gate + API malformed fuzz gate + API semantic property gate + CLI_VERSION gate | 继续按高风险变更补 oracle/mutation；侧信道保留正式 dudect/后端产物级审计边界 | P0/P1 |
| L1 | 集成测试 | ~15个 | +10（跨包闭环） | P1 |
| L2 | 属性测试 | 3个 | +8（Ed25519/Fe/canonjson扩展） | P1 |
| L3 | 差分测试 | 固定夹具 | +随机差分harness（Ed25519/SHA/HMAC） | P1 |
| L4 | CLI黑盒 | ps1=54, sh=54 | 后续改 CLI 时双脚本同步 | P1 |
| L5 | 变异测试 | 16点 | 继续按高风险改动补点 | P1 |
| L6 | 模糊测试 | 无 | 10000轮随机输入不崩溃 | P2 |
| L7 | 性能/基准 | 仅js | +native/wasm-gc + Ed25519基准 | P2 |
| L8 | 安全测试（侧信道） | 源码级常量时间审计 + JS verify timing sampler + native verify/sign dudect-style sampler | 专业 dudect/后端机器码审计前不作生产级侧信道声明 | P1 |
...
**仍需完成**：Phase 1 测试项与 CT-001 源码级修复已收口；native timing 经验性证据已补。生产级侧信道声明仍需正式 dudect/后端产物审计。
...
| 2.4 | bash cli-test补齐Part4+Part5（12例） | L4 | Done: `cli-test.sh` now covers 54/54 cases and CI runs bash parity for native/js |
...
**验收门禁**：
- 变异测试捕获率 16/16 = 100%
- bash 和 PowerShell CLI 用例数一致（54/54）
...
| 3.5 | 动态时序测量（Ed25519验证/签名采样+统计） | L8 | Done: JS `tools/timing-ed25519-verify.mjs --samples 10000` 记录 Welch t=-0.118306；native `tools/timing-ed25519-native.ps1 -Target both -Samples 50000 -Warmup 1024 -Config release` 覆盖 verify/sign-message/sign-secret，Welch t 分别为 -0.147045 / 0.090476 / -0.040215。明确不是形式化证明或正式 dudect 审计 |
...
- `reduce_scalar_512` 的 CT-001 已在源码级修复；native 动态时序长跑未观察到明显 timing difference；仍需后端产物/正式 dudect 审计，才能作生产级侧信道声明。
...
| RISK-005 | bash CLI测试盲区导致CI通过但行为退化 | 低 | 高 | 已补齐到54例；CI native/js 同跑 bash 与 PowerShell |
...
| 三后端单元回归 | `moon test --target native`; `moon test --target js`; `moon test --target wasm-gc` | PASS：344/344 + 344/344 + 344/344 | 每次 PR / 发布候选 |
| CLI 黑盒合同 | `powershell -ExecutionPolicy Bypass -File tools/cli-test.ps1 -Target native/js`; `bash ./tools/cli-test.sh native/js` | PASS：native 54/54 + js 54/54（PowerShell 与 bash） | 每次 PR / 发布候选 |
...
| 动态时序探针 | `node tools/timing-ed25519-verify.mjs --samples 10000`; `powershell -ExecutionPolicy Bypass -File tools/timing-ed25519-native.ps1 -Target both -Samples 50000 -Warmup 1024 -Config release` | JS 10000 样本已记录；native 50000 长跑 verify/sign-message/sign-secret 均 `|t| < 4.5`；信息性结果，不是形式化证明 | 手动安全审计 |

剩余边界也固定为文档化风险：Ed25519 侧信道已有本机 native timing 经验性证据，但仍缺正式 dudect/后端产物级证明；`@fs` 无 lstat/symlink API，符号链接语义只能靠 depth/file cap 与 create depth-cap abort 缓解；随机测试是高强度抽样，不是全输入证明；CI 仍需在远端 ubuntu/gcc 环境确认 native gate。
...
| 2026-07-07 | native timing evidence：新增 `src/timing` 与 `tools/timing-ed25519-native.ps1`，本机 MSVC native release 长跑 50000 样本覆盖 verify/sign-message/sign-secret，三项 `|t| < 4.5`；仍声明不是形式化证明或正式 dudect 审计 | Codex |
