...
当前规模：12 个产品包 + 1 个 native timing 工具包；产品实现 6547 行，测试 8430 行，总计 14977 行 MoonBit，产品实现规模符合 4-10k 参考区间。

测试体系：353 个可执行 MoonBit 测试、357 个测试声明、68 个 CLI 黑盒用例，覆盖 RFC 8032 / Wycheproof / NIST / RFC 8785 向量、随机差分、变异验证、malformed fuzz、分支审计和 stale-check 门禁；Fabric Chaincode 覆盖率 82.1%，Gateway 19/19。
...
开源规范：仓库包含 Apache-2.0 `LICENSE`、中文 README / English README、SECURITY、CONTRIBUTING、CHANGELOG、开发报告、验收清单、决策日志和结果日志。
