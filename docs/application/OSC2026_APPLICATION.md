...
提供沉浸式产品首页，用四章滚动叙事展示“材料进入、形成凭证、修改分叉、定位拒绝”的可信校验闭环；“开始验证”进入独立原生 React 工作台，覆盖验证、创建、证明、审计、签名和篡改实验六个视图，共享一个 Web Worker 和 12 个 MoonBit API，无 iframe、无后端；
...
项目以可复跑测试、可解释诊断、滚动可视化首页和真实证据工作台作为展示入口，体现 MoonBit 在多后端、强类型和工程化交付上的综合能力。
