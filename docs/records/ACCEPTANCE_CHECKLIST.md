...
| 序号 | 验收项 | 结论 | 可复核证据 |
| --- | --- | --- | --- |
| 1 | 项目以 MoonBit 为主要实现语言 | 通过 | 证据模型、规范化、摘要、Merkle、创建、验证、诊断、签名和 CLI 语义均位于 [`src/`](../../src/)。当前实现 6547 行 + 测试 8430 行，合计 **14977 行**；12 个产品包和 1 个原生计时工具包由 [`moon.mod`](../../moon.mod) 管理。Go/TypeScript 只承担 Fabric 协议适配，不重算证据语义 |
...
| 8 | 发布到 mooncakes.io | 通过 | [`starlittle/MoonEvidence` v0.5.1](https://mooncakes.io/docs/#/starlittle/MoonEvidence)已发布；`moon.mod`、CLI 版本、CHANGELOG、Git tag 和注册表版本一致；包内容门禁逐项检查发布清单，并排除 Fabric、比赛材料和本地记录 |
...
| 证据 | 当前记录 |
| --- | --- |
| 发布版本 | [GitHub Release v0.5.1](https://github.com/wenlittle/MoonEvidence/releases/tag/v0.5.1)，GitHub/GitLink 注解标签均指向同一发布提交 |
...
| Fabric | [发布双组织记录](fabric-e2e/2026-07-12-v0.5.1/)保存下载后的 GitHub Release 与 Mooncakes 制品哈希、block 16 `VALID` 交易、跨组织查询、幂等重复和 `E2003`/`E2004` 回传 |
