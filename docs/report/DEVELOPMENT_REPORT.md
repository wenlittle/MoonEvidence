...
> 仓库：https://github.com/starlittle/MoonEvidence ｜ 规模：12607 行 MoonBit（实现 5448 + 测试 7159）｜ 提交：109 个 ｜ 包：12 个
...
| 层 | 数量与内容 |
| --- | --- |
| 单元测试 | **344 个**测试声明（340 个可执行测试 + 4 个基准 wrapper），wasm-gc 与 js 双后端全绿；含 NIST/RFC/Wycheproof 向量、JCS fixtures、版本链图语义、Merkle 树物化与路径 |
...
| 测试行数 | 7159 |
| 总行数 | **12607** |
| 测试声明 | 344（340 测试 + 4 基准调用） |
| 单元测试通过 | **340/340**（wasm-gc + js 双后端） |
| CLI 黑盒通过 | **53/53** |
| 包数 | **12** |
