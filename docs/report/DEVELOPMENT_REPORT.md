...
| 检查项 | 结果 |
| --- | --- |
| 发布文件 | GitHub Release `starlittle-MoonEvidence-0.5.1.zip`，SHA-256 `ab0dd9b6…7b5bd3` |
| 首次提交 | 交易 `f004f179…965c2d` 在 block `16` 以 `VALID` 提交 |
...
%%{init: {"theme": "neutral"}}%%
flowchart TB
    A["block 16 GitHub Release manifest 摘要"]
    B["当前证据包"]
    B --> C["只修改文件"]
    C --> D["manifest 仍是原值"]
    D --> E["文件摘要检查返回 E2003"]
