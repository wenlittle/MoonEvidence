...
| MoonBit 源码 | **14,667** 行（实现 6,529 + 测试 8,138） |
| MoonBit 包 | **13** 个（12 个产品包 + 1 个原生计时工具包） |
| 测试声明 | **352** 个（348 个可执行测试 + 4 个基准包装） |
| CLI 黑盒 | PowerShell、bash 各 `67/67`，覆盖 JS 和 native |
...
| 验证面 | 结果 |
| --- | --- |
| MoonBit 测试 | 348/348，native、wasm、wasm-gc、js 全部通过 |
| CLI | PowerShell/bash 各 67/67，JS/native 对等 |
